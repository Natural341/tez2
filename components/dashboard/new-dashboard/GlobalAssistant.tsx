...
const GlobalAssistant: React.FC<GlobalAssistantProps> = ({ currentPage }) => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'init',
      role: 'assistant',
      content: 'Hello. I am InsightFlow Secure Analysis Assistant. I am ready to examine anomalies or trends in your datasets. How can I help you?',
      timestamp: new Date()
    }
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const context = `User is currently viewing the ${currentPage} page.`;
      ...
      >
        ...
        <div className="bg-slate-900 p-4 flex justify-between items-center text-white shrink-0">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h3 className="font-semibold text-sm">InsightFlow AI</h3>
                    <div className="flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse"></span>
                        <p className="text-[10px] text-slate-300 font-medium tracking-wide uppercase">Online • Secure Line</p>
                    </div>
                </div>
            </div>
