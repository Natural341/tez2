...
import { AnalysisType } from '@/types/new-dashboard';
...
const Header: React.FC<HeaderProps> = ({ isLive, toggleLive, onSearch, analysisType, setAnalysisType, toggleSidebar }) => {
  ...
  return (
    <header className="h-16 bg-white border-b border-zinc-100 flex items-center justify-between px-4 md:px-6 sticky top-0 z-20 shadow-sm transition-all duration-300">
      ...
      <div className="flex items-center gap-3 md:gap-4">
        ...
        <div className="relative group hidden sm:block">
            <select 
                ...
                className="appearance-none bg-zinc-50 border border-zinc-200 text-zinc-700 text-sm font-semibold py-2 pl-4 pr-10 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300 cursor-pointer hover:bg-zinc-100 transition-colors"
            >
      ...
      <div className="flex-1 max-w-xs md:max-w-lg mx-4">
        <form onSubmit={handleSearchSubmit} className="relative group">
          ...
          <input
            ...
            className="w-full bg-zinc-50 border border-zinc-200 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-700 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-100 focus:bg-white transition-all"
          />
        </form>
      </div>
