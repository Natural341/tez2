...
import { analyzeDataQuality, generateSQLFromText } from '../../services/geminiService';
import { MockRow, DataQualityIssue } from '../../types';
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12 h-full flex flex-col">
      <div className="flex justify-between items-end mb-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">İnovasyon Laboratuvarı</h2>
          <p className="text-slate-500 mt-1">Deneysel AI araçları ve prototip özellikler.</p>
        </div>
        <div className="flex bg-slate-100 p-1 rounded-lg">
            ...
            >
                <div className="flex items-center gap-2"><Wand2 size={16}/> Veri Temizleyici</div>
            </button>
            <button 
            ...
            >
                 <div className="flex items-center gap-2"><SlidersHorizontal size={16}/> Simülasyon</div>
            </button>
            <button 
        ...
        <div className="flex flex-col h-full gap-6">
            <div className="flex justify-between items-center bg-indigo-50 p-4 rounded-lg border border-indigo-100">
                <div>
                    ...
                    <p className="text-sm text-indigo-700 mt-1">AI, eksik verileri ve anomalileri tespit eder ve onarır.</p>
                </div>
                <button 
            ...
            <div className="flex gap-6 flex-1 min-h-0">
                ...
                <div className="w-80 border-l border-slate-100 pl-6 flex flex-col">
                    <h4 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
                        <Check size={16} className="text-emerald-500" /> Önerilen Düzeltmeler
                    </h4>
                    ...
                    <div className="space-y-3 overflow-y-auto pr-2">
                        {issues.length === 0 && !analyzing && (
                            <div className="text-center p-6 bg-slate-50 rounded-lg border border-slate-200 border-dashed">
                                <p className="text-slate-500 text-sm">Sorun bulunamadı veya analiz henüz başlatılmadı.</p>
                            </div>
                        )}
    ...
    return (
        <div className="flex h-full gap-8">
            <div className="w-1/3 space-y-8">
                <div>
                    <h3 className="font-bold text-slate-900 text-lg">What-If Simülatörü</h3>
                    <p className="text-sm text-slate-500 mt-1">Parametreleri değiştirerek gelecekteki olası senaryoları modelleyin.</p>
                </div>
                ...
                <div className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                    <h4 className="text-sm font-bold text-slate-900 mb-2">Özet Tahmin</h4>
                    <div className="flex justify-between items-center">
                        <span className="text-slate-500 text-xs">24. Ay Değeri</span>
                    ...
                    <p className="text-[10px] text-slate-400 mt-2 text-justify">
                        *Monte Carlo simülasyonu, volatilitiye dayalı rastgele değişkenler kullanarak hesaplanmıştır.
                    </p>
                </div>
            </div>

            <div className="flex-1 bg-slate-50/50 rounded-xl border border-slate-100 p-4 relative">
                 <h4 className="absolute top-4 left-4 text-xs font-bold text-slate-400 uppercase tracking-wide z-10">Projeksiyon Grafiği</h4>
        ...
        <div className="h-full flex flex-col gap-6">
             <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Doğal dil ile veritabanı sorgusu oluşturun.</p>
                </div>
             </div>

             <div className="flex gap-6 flex-1 min-h-0">
                <div className="w-1/2 flex flex-col gap-4">
                    <div className="flex-1 bg-slate-50 border border-slate-200 rounded-lg p-4 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                        <textarea 
                            value={query}
                            ...
                            className="w-full h-full bg-transparent resize-none focus:outline-none text-slate-700 placeholder:text-slate-400 text-sm leading-relaxed"
                        />
                    </div>
                    <button 
                        onClick={handleGenerate}
                        disabled={loading || !query.trim()}
                        className="w-full bg-slate-900 hover:bg-slate-800 text-white font-medium py-3 rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-70"
                    >
                ...
                <div className="w-1/2 bg-[#1e1e1e] rounded-lg p-4 overflow-auto relative group">
                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                         <button className="text-xs bg-white/10 text-white px-2 py-1 rounded hover:bg-white/20">Kopyala</button>
                    </div>
                    <pre className="font-mono text-sm text-emerald-400 whitespace-pre-wrap">
                        {generatedSQL || "-- SQL çıktısı burada görünecek..."}
                    </pre>
                </div>
             </div>
        </div>
